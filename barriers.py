...
class Barrier:
    ...
    def __init__(self, game, rect):
        ...
        li = [
            BarrierPiece(game, Barrier.randcolor(), x + j * w, y + i * h, w, h)
            for j in range(factor)
            for i in range(factor)
            if not Barrier.top_corners(i, j, factor)
            and not Barrier.mid_arch(i, j, factor)
        ]
    ...
    def update(self):
        collisions = pg.sprite.groupcollide(
            self.alien_lasers.lasergroup(), self.pieces, True, True
        )
        collisions = pg.sprite.groupcollide(
            self.ship_lasers.lasergroup(), self.pieces, True, True
        )
        self.draw()
...
class Barriers:
    ...
    def create_barriers(self):
        ...
        self.barriers = [
            Barrier(
                game=self.game,
                rect=pg.Rect(i * 2 * width + 1.5 * width, top, width, height),
            )
            for i in range(4)
        ]  # SP w  3w  5w  7w  SP
