...
class Laser(Sprite):
    ...
    def update(self):
        ...
        if self.owner.laser_offscreen(self.rect):
            self.kill()  # delete off-screen lasers
        self.draw()
...
class Lasers:  # can now have aliens or ship firing
    def __init__(
        self, game, timer, v, owner
    ):  # one instance for Ship and one for Aliens
        self.game = game
        self.screen = game.screen
        self.settings = game.settings
...
if __name__ == "__main__":
    print("\nERROR: lasers.py is the wrong file! Run play from game.py\n")
